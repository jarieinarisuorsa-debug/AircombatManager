      ...
      <div class="ui-table-container">
        ...
        <table class="matrix-table editor-matrix-table">
          ...
          <tbody>
            ...
            <!-- AIRCRAFT & SIGNATURES -->
            ${isWWI ? renderModelPointsRow("Mallipisteet", stages, card, totals) : renderInteractiveBooleanRow("2,5-luokka", stages, card, totals, "twoPointFiveClass", "dummy", entry.id, false, true)}
...
function renderModelPointsRow(label, stages, card, totals) {
  return `
    <tr class="matrix-data-row">
      <td class="sticky-col"><strong>${label}</strong></td>
      ${stages.map(stage => {
        const roundScore = totals?.roundScores?.find(rs => rs.roundNumber === stage.roundNumber)?.score;
        const points = roundScore?.modelPoints !== undefined ? roundScore.modelPoints : "";
        
        return `
          <td>
            <div class="print-input-group" style="justify-content: center; align-items: center; margin-top: 8px;">
              <div class="print-labels"><span>pisteet</span></div>
              <div class="print-boxes" style="margin-top: 2px;">
                <div class="print-box yellow-box" style="width: 100%; font-weight: bold;">${points}</div>
              </div>
            </div>
          </td>
        `;
