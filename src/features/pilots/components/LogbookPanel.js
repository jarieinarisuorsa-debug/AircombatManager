...
export function renderLogbookPanel(state, pilot) {
  ...
  const classStatsHtml = logbookTab === 'Yhteenveto' ? Object.entries(stats.classes).map(([className, cls]) => `
    <article class="small-card" style="flex: 1; min-width: 160px; background: rgba(88, 183, 255, 0.04); border: 1px dashed rgba(88, 183, 255, 0.2);">
      ...
      <div style="display: flex; justify-content: space-between; font-size: 0.85rem; color: var(--muted);">
        <span>Kierroksia: <strong style="color: var(--text);">${cls.flights}</strong></span>
        <span>Cutteja: <strong style="color: var(--text);">${cls.cuts}</strong></span>
      </div>
    </article>
  `).join('') : "";
  ...
  const summaryPanel = `
    <div style="margin-bottom: 32px;">
      <h3 style="margin-bottom: 16px; display: flex; align-items: center; gap: 8px; font-size: 1.2rem;">
        ...
        ${logbookTab === 'Yhteenveto' ? "Kaikkien luokkien yhteenveto" : `Yhteenveto: ${escapeHtml(logbookTab)}`}
      </h3>
      <div class="result-stat-grid" style="margin-bottom: 16px;">
        <article class="small-card"><span>Kilpailut</span><strong>${summaryStats.events}</strong></article>
        <article class="small-card"><span>Kierrokset</span><strong>${summaryStats.flights}</strong></article>
        <article class="small-card"><span>Cutit</span><strong style="color: var(--accent);">${summaryStats.cuts}</strong></article>
        <article class="small-card"><span>Lentoaika</span><strong>${Math.floor(summaryStats.flightSeconds / 60)}<small style="font-size:0.6em; margin-left:4px; color: var(--muted);">min</small></strong></article>
      </div>
      ${classStatsHtml ? `<div style="display: flex; gap: 12px; flex-wrap: wrap;">${classStatsHtml}</div>` : ""}
    </div>
  `;

  const historyHtml = filteredEvents.length === 0 ? `<p class="muted">Ei tuloksia tässä luokassa.</p>` : filteredEvents.map(ev => {
    const flightsHtml = ev.flights.map(f => `
      <tr style="border-bottom: 1px solid var(--border); font-size: 0.95rem; transition: background 0.2s ease;">
        ...
        <td style="padding: 12px 20px;">${f.streamerOk ? `<span style="color: #4ade80;">Ehjä</span>` : `<span class="muted">-</span>`}</td>
      </tr>
    `).join('');

    const statusBadge = ev.resultsPublished 
      ? `<span class="badge badge-saved" style="font-size: 0.75rem;">Virallinen</span>` 
      : `<span class="badge badge-empty" style="font-size: 0.75rem;">Luonnos</span>`;
        ...
        <div style="background: linear-gradient(135deg, rgba(88,183,255,0.08), transparent); padding: 20px; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
          <div>
            ...
            <div style="display: inline-flex; gap: 8px;">
              <span class="badge" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); font-weight: 500;">Luokka: <span style="color: var(--accent); margin-left: 4px;">${escapeHtml(ev.className)}</span></span>
              ${ev.aircraftName ? `<span class="badge" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); font-weight: 500;">Kone: <span style="color: var(--text); margin-left: 4px;">${escapeHtml(ev.aircraftName)}</span></span>` : ""}
            </div>
          </div>
          <div style="text-align: right; background: rgba(0,0,0,0.3); padding: 14px 20px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.05); min-width: 140px;">
            ${ev.resultsPublished && ev.ranking ? `<div style="font-size: 0.85rem; color: var(--muted); margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em;">Sijoitus</div><div style="font-size: 1.6rem; font-weight: 800; color: var(--text); line-height: 1;">${ev.ranking}.</div>` : ""}
            ${ev.resultsPublished && ev.ranking ? `<div style="height: 1px; background: rgba(255,255,255,0.1); margin: 10px 0;"></div>` : ""}
            ${ev.resultsPublished ? `<div style="font-size: 0.85rem; color: var(--muted); margin-bottom: 2px; text-transform: uppercase; letter-spacing: 0.05em;">Pisteet</div><div style="font-size: 1.2rem; font-weight: bold; color: var(--accent);">${ev.totalEventScore}</div>` : ""}
            ${!ev.resultsPublished ? `<div style="color: var(--muted); font-size: 0.9rem;">Odottaa julkaisua</div>` : ""}
          </div>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse; text-align: left; margin: 0;">
            <thead style="background: rgba(0,0,0,0.2);">
              <tr style="font-size: 0.8rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em;">
                <th style="padding: 14px 20px; font-weight: 600;">Kierros</th>
                <th style="padding: 14px 20px; font-weight: 600;">Pisteet</th>
                <th style="padding: 14px 20px; font-weight: 600;">Cutit</th>
                <th style="padding: 14px 20px; font-weight: 600;">Aika</th>
                <th style="padding: 14px 20px; font-weight: 600;">Streamer</th>
              </tr>
            </thead>
            <tbody>
              ${flightsHtml}
