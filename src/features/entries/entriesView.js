...
export function renderEntriesView(state) {
  ...
  const tabNav = UI.ScrollableNav({
    ...
    <button type="button" class="button ${tab === 'ilmoittautumiset' ? 'nav-active' : 'dashed'}" style="flex-shrink: 0;" data-action="set-workspace-tab" data-tab="ilmoittautumiset">${t(state, "event_workspace.tab_registrations")}</button>
    <button type="button" class="button ${tab === 'luokka' ? 'nav-active' : 'dashed'}" style="flex-shrink: 0;" data-action="set-workspace-tab" data-tab="luokka">${t(state, "event_workspace.tab_class")}</button>
    <button type="button" class="button ${tab === 'rakenne' ? 'nav-active' : 'dashed'}" style="flex-shrink: 0;" data-action="set-workspace-tab" data-tab="rakenne">${t(state, "event_workspace.tab_format")}</button>
    <button type="button" class="button ${tab === 'osallistujat' ? 'nav-active' : 'dashed'}" style="flex-shrink: 0;" data-action="set-workspace-tab" data-tab="osallistujat">${t(state, "event_workspace.tab_all_pilots")}</button>
    <button type="button" class="button ${tab === 'kilpailijat' ? 'nav-active' : 'dashed'}" style="flex-shrink: 0;" data-action="set-workspace-tab" data-tab="kilpailijat">${t(state, "event_workspace.tab_competitors")}</button>
