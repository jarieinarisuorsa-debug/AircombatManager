...
export function handleScoreCardInput(form) {
  ...
  template.rounds.forEach((roundNumber) => {
    ...
    const flightTotalDisplay = form.querySelector(`[name="r${roundNumber}_flightTotal_display"]`);
    if (flightTotalDisplay) flightTotalDisplay.textContent = score.flightTotal;

    const totalInput = form.querySelector(`input[name="r${roundNumber}_total"]`);
    if (totalInput) totalInput.value = score.total;

    const modelPointsDisplay = form.querySelector(`[name="r${roundNumber}_modelPoints_display"]`);
    if (modelPointsDisplay) modelPointsDisplay.textContent = score.modelPoints;
