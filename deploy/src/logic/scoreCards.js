...
export function createDefaultScoreCard({ entry, pilot, aircraft, event }) {
  ...
  return {
    ...
    frequency: "2.4 GHz",
    flyingRound: "",
    rounds: stages.map((stage) => createDefaultRound(stage.roundNumber, templateId)),
    aircraft: {
      twoPointFiveClass: false,
      modelName: aircraft?.name || "",
      motorOrBattery: aircraft?.engine || "",
      propeller: "",
      rpm: ""
    },
    signatures: {
...
export function createDefaultRound(roundNumber, templateId = SCORE_CARD_TEMPLATE_STANDARD) {
  return {
    roundNumber,
    modelPoints: createDefaultModelPoints(),
    takeoff: false,
    flightMinutes: 0,
    flightSeconds: 0,
    cuts: 0,
...
export function calculateScoreCardRound(round, event, templateOrId = null) {
  ...
  const total = Math.max(0, modelPoints + takeoffPoints + flightPoints + cutPoints + groundTargetPoints + landingAfterEndSignalPoints + streamerOkPoints - penaltyPoints);
...
export function buildScoreCardRows(state, event) {
  ...
  return state.entries
    ...
    .map((entry) => {
      ...
      const calculatedFlyingRound = pilotHeats.map(h => h.groupName.split(" ").pop()).join(", ");
