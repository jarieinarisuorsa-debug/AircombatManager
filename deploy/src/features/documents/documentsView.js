...
export function renderDocumentsView(state) {
  ...
  const pageHeader = UI.PageHeader({
    kicker: "Admin · Asiakirjat",
    title: "Tulostettavat asiakirjat",
    subtitle: `Tulosta kilpailupaikan paperidokumentit kisaan: ${activeEvent.name}`,
    className: "no-print"
  });
  ...
  if (tab === "tuloskortit") {
    ...
    contentHtml = UI.Panel({ title: "Aikataulut" }, `
      <div style="display: flex; flex-direction: column; gap: 15px;">
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; background: var(--surface-2); border-radius: 6px;">
          <div>
...
function getAvailableClasses(state, activeEvent) {
  ...
  if (unique.length === 0) return ["WW2", "WWI", "EPA"];
  return unique.sort();
}
