...
export function renderEntriesView(state) {
  ...
  const editorRow = state.settings?.scoreCardEditorOpen
    ? buildScoreCardRows(state, activeEvent).find((row) => row.entry.id === state.settings.scoreCardEditorEntryId)
    : null;
  const editorModal = editorRow ? renderScoreCardEditorModal(activeEvent, editorRow) : "";

  return [workspaceHero, tabNav, content, editorModal].join("");
}
...
function renderWorkspaceParticipantsTab(state, activeEvent, className) {
  ...
  const searchContainer = UI.Flex({ justify: "space-between", align: "center", wrap: "wrap", gap: "10px", className: "pilot-search-container", style: "margin-bottom: 20px;" }, `
    ${UI.Flex({ gap: "10px", style: "flex: 1; min-width: 300px;", className: "search-input-wrapper" }, `
      <input type="search" id="workspace-pilot-search" value="${escapeHtml(state.settings?.workspacePilotSearch || "")}" placeholder="Etsi nimellä tai seuralla..." autocomplete="off" style="flex: 1; max-width: 300px; min-height: 34px; padding: 6px 12px; border-radius: 8px;" />
      ${UI.Select({ 
...
function renderWorkspaceHeatsTab(state, activeEvent, className) {
  ...
  return UI.Panel({ kicker: "Vaihe 4", title: `${escapeHtml(className)}: Heat-arvonta ja listat` }, `
    ${controls}
    ${heats.length ? renderClassHeatSection(state, entries, className, heats, admin) : "<p class='muted'>Ei arvottuja heateja.</p>"}
  `);
}
...
function renderWorkspaceScorecardsTab(state, activeEvent, className) {
  ...
  return UI.Panel({ kicker: "Vaihe 5", title: `${escapeHtml(className)}: Tuloskortit` }, renderScoreCardList(rows, className));
}
