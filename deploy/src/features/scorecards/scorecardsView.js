...
import { renderScoreCardEditorModal } from "./components/ScoreCardEditor.js";
...
export function renderScoreCardsView(state) {
  ...
  const editorRow = state.settings?.scoreCardEditorOpen
    ? rows.find((row) => row.entry.id === state.settings.scoreCardEditorEntryId)
    : null;
  ...
  return `
    ${pageHeader}
    ${statsPanel}
    ${renderScoreCardList(rows, targetClass)}
    ${editorModal}
  `;
}
