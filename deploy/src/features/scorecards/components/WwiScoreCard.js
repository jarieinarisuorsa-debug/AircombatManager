  ...
  return `
    <div class="score-sheet-layout-wwi">
      <div class="score-sheet-grid score-sheet-grid-wwi" style="${gridStyle}" aria-label="WWI pilot score card">
        ...
        <div class="score-section-title">Konepisteet</div>
        ${getRounds(card, template).map(() => `<div class="score-section-title small">kyllä &nbsp;&nbsp; ei</div>`).join("")}
        <div class="score-section-title small">max 100</div>
        ${template.modelPointItems.map((item) => renderWwiModelPointRow(card, template, item)).join("")}

        <div class="score-section-title">Lentopisteet</div>
        ${getRounds(card, template).map(() => `<div class="score-section-title small">kyllä &nbsp;&nbsp; ei</div>`).join("")}
        <div class="score-section-title small"></div>
        ...
        ${renderWwiNumberRow(card, template, "Maamaalit", "groundTargets", "50 p")}
