...
export function saveSettings(data, form) {
  updateState((state) => {
    ...
    if (data.adminEmails !== undefined) {
      state.settings.adminEmails = String(data.adminEmails)
        .split(",")
        .map(e => e.trim())
        .filter(e => e.length > 0);
    }
    
    if (data.publicEmails !== undefined) {
      state.settings.publicEmails = String(data.publicEmails)
        .split(",")
        .map(e => e.trim())
        .filter(e => e.length > 0);
    }
...
export function initSettingsActions() {
  ...
  registerAction("logout-user", (event, button, { renderApp }) => {
    logoutUser();
    renderApp();
    return true;
  });
