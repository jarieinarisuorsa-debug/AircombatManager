...
export function renderHeatsView(state) {
  ...
  const heatsHtml = heats.length 
    ? Array.from(heatsByClass.entries()).map(([className, classHeats]) => renderClassHeatSection(state, entries, className, classHeats, admin)).join("")
    : UI.Panel({
...
export function renderClassHeatSection(state, entries, className, classHeats, admin) {
  ...
  return `
    ...
    ${phases.map((phase) => {
      ...
      return `
        ...
        ${phaseHeats.map((heat) => renderHeatCard(state, entries, heat, admin)).join("")}
      `;
    }).join("")}
  `;
}

export function renderHeatCard(state, entries, heat, admin, highlightPilotId = null) {
  ...
  const list = `
    <ol class="pilot-list">
      ${heat.entryIds.map((entryId) => renderHeatPilotRow(state, entries.find((item) => item.id === entryId), conflicts.has(entryId), admin, highlightPilotId)).join("")}
    </ol>
  `;
...
function renderHeatPilotRow(state, entry, hasConflict, admin, highlightPilotId = null) {
  if (!entry) return "";
    ...
    <li style="display: flex; justify-content: space-between; align-items: center; padding: 4px 0;">
      <div style="display: flex; align-items: center;">
        ...
        <div style="display: flex; flex-direction: column;">
          ...
          <span>${admin ? `${escapeHtml(entry.className)} · ${escapeHtml(getAircraftName(state, entry.aircraftId))} (${escapeHtml(freq)})` : `${escapeHtml(entry.className)} (${escapeHtml(freq)})`}</span>
        </div>
      </div>
    </li>
  `;
