...
export const UI = {
  ...
  // Forms & Inputs
  FormPanel: ({ title = "", kicker = "", action = "", id = "", className = "", style = "" } = {}, content = "") => {
    return `
      <form class="panel form-panel ${className}" ${id ? `id="${escapeHtml(id)}"` : ""} ${action ? `data-action="${escapeHtml(action)}"` : ""} ${style ? `style="${escapeHtml(style)}"` : ""}>
        ${kicker ? `<p class="kicker">${escapeHtml(kicker)}</p>` : ""}
